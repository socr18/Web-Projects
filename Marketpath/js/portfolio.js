...
    var success = function (data) {         //data downloaded so we call parseJSON function
               //and pass downloaded data

        var json = $.parseJSON(data);
        arr = $.parseJSON(data);

        for (var i = 0; i < json.length; i++) {         //console.log(json);
                     //add images to gallery

            $('.bxslider2').append('<li data-order="' + arr[i]['sort-service'] + '"> <a href="' + json[i]['page-url'] + '"><h2>' + arr[i]['client-name'] + '</h2><img src="' + json[i]['website-url'] + '" alt="' + json[i]['client-name'] + '" /></a></li>');

            if (json[i]['sort-industry'] > 0) {

                $('.featured-logo').append('<div class="my-5 columns" data-order="' + json[i]['sort-industry'] + '"><a href="/' + json[i]['page-url'] + '"><img src="' + json[i]['logo-url'] + '" alt="" /></a></div>');
...
                $('.bxslider2').append('<li data-order="' + arr[i]['sort-service'] + '"> <a href="' + arr[i]['page-url'] + '"><img src="' + arr[i]['website-url'] + '" alt="' + arr[i]['client-name'] + '" /><h2>' + arr[i]['client-name'] + '</h2></a></li>');
            }
        }
